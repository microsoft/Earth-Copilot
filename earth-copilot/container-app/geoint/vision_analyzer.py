        ...
        try:
            ...
            if cache_key in self._imagery_cache:
                ...
                if age < self._cache_ttl:
                    ...
                    logger.info(f"⏰ Cache expired (age: {int(age)}s), fetching fresh imagery")
            ...
            if not items:
                logger.warning("⚠️ No recent Sentinel-2 imagery found in 30-day window")
            ...
            logger.info(f"⬇️ Downloading imagery tile...")
                ...
                async with session.get(signed_tile_url, timeout=aiohttp.ClientTimeout(total=45)) as response:
                    if response.status == 200:
                        ...
                        if len(self._imagery_cache) > 50:
                            ...
                            logger.debug(f"♻️ Cache evicted oldest entry (cache size: {len(self._imagery_cache)})")
