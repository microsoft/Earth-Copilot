...
class GeospatialTools:
    ...
    async def calculate_terrain_slope(
        ...
        """
        try:
            logger.info(f"🏔️ Calculating terrain slope for ({latitude:.6f}, {longitude:.6f}), radius={radius_miles}mi")
    ...
    async def classify_land_cover(
        ...
        """
        try:
            logger.info(f"🗺️ Classifying land cover for ({latitude:.6f}, {longitude:.6f})")
    ...
    async def _download_raster(
        ...
        try:
            ...
            if not items:
                logger.warning(f"⚠️ No STAC items found for {collection_id}")
                return None
            ...
            if asset_key not in item.assets:
                ...
                logger.warning(f"⚠️ Asset {asset_key} not found. Available: {available_assets}")
                if available_assets:
