...
# Global TerrainAgent instance for session persistence across requests
_terrain_agent_instance = None

def _get_terrain_agent():
    """Get or create singleton TerrainAgent instance."""
    global _terrain_agent_instance
    if _terrain_agent_instance is None:
        from .terrain_agent import TerrainAgent
        _terrain_agent_instance = TerrainAgent()
        logger.info("🏔️ Created global TerrainAgent singleton")
    return _terrain_agent_instance
...
async def terrain_analysis_agent(
    ...
    """
    🏔️ Terrain Analysis Agent - Full SK Agent with DEM tools + GPT-4o Vision
