...
"""
    ...
    async def _ensure_kernel_initialized(self):
        ...
        try:
            ...
            # Add Azure OpenAI service with optimal GPT-4o configuration for SK 1.37.0+
            # Use stable API version that works well with GPT-4o
            ...
            # Primary service: GPT-4o for complex reasoning tasks
            if ad_token:
                azure_chat_service = AzureChatCompletion(
                    deployment_name=self.model_name,
                    ad_token=ad_token,
                    base_url=base_url,
                    api_version="2024-10-21",  # Stable API version with full GPT-4o support
                    service_id="chat-completion"  # Explicitly set service ID
                )
                logger.info(f"   ✅ AzureChatCompletion service created with managed identity (GPT-4o)")
            else:
                azure_chat_service = AzureChatCompletion(
                    deployment_name=self.model_name,
                ...
                logger.info(f"   ✅ AzureChatCompletion service created with API key (GPT-4o)")
            
            self.kernel.add_service(azure_chat_service)
            logger.info(f"   ✅ GPT-4o service added to kernel")
