...
COLLECTION_PROFILES = {
    ...
    "hls2-l30": {
            ...
            "query_rules": {
                    ...
                    "agent_guidance": """
🌟 DEFAULT COLLECTION: HLS should be your FIRST CHOICE for optical imagery queries unless user explicitly requests a different collection.

WHY HLS IS THE BEST DEFAULT:
✓ MAXIMUM COVERAGE: Combines Landsat 8/9 + Sentinel-2A/B/C for 2-3 day revisit (vs 5-16 days individual)
✓ ANALYSIS-READY: Atmospherically corrected, BRDF-normalized, harmonized processing
✓ GLOBAL: Available worldwide (unlike NAIP which is USA-only)
✓ CONSISTENT: Same 30m resolution and band definitions across all sensors
✓ DEEP ARCHIVE: 12+ years of consistent data (April 2013 - Present)

USE HLS FOR:
✓ Generic requests like "show me images of Seattle" or "imagery of Paris in October"
✓ Time-series analysis and change detection
✓ Agriculture and crop monitoring
✓ Any query without specific resolution requirements
...
1. STATIC COLLECTIONS (Elevation/DEM data - NO temporal dimension):
   ...
   ✓ ONLY use: bbox, limit
   ✗ NEVER use: datetime, cloud_cover
...
2. COMPOSITE COLLECTIONS (Pre-aggregated with clouds removed):
   ...
   ✓ ONLY use: bbox, sortby, limit
   ✗ NEVER use: datetime (use sortby instead), cloud_cover (already filtered)
...
3. DYNAMIC COLLECTIONS (Full time-series data):
   ...
   ✓ CAN use: ALL standard STAC parameters
   ✓ datetime, cloud_cover (if optical), bbox all supported
