...
export class TileUrlGenerator {
  ...
  static generateItemTileUrl(options: TileUrlOptions): string {
    ...
    // Build base URL
    let url = `${this.PC_BASE_URL}/item/tiles/WebMercatorQuad/{z}/{x}/{y}@${scale}x`;
    ...
    if (config) {
      // Handle radar collections with rescaling
      if (options.collection === 'sentinel-1-rtc') {
        if (!options.rescale) {
          params.set('rescale', '-30,0');
        }
      }
      
      // Handle climate data with colormaps
      ...
      if (['nasadem', 'cop-dem-glo-30'].includes(options.collection)) {
        if (!options.colormap) {
          params.set('colormap', 'terrain');
        }
      }
      
      // Handle Landsat thermal infrared
      if (options.collection === 'landsat-c2-l2' && assets.includes('lwir11')) {
        if (!options.colormap) {
          params.set('colormap', 'thermal');
        }
        if (!options.rescale) {
          params.set('rescale', '250,350');
        }
      }
    }
    ...
    if (options.return_mask !== undefined) params.set('return_mask', options.return_mask.toString());
    if (options.algorithm) params.set('algorithm', options.algorithm);
    if (options.algorithm_params) {
      Object.entries(options.algorithm_params).forEach(([key, value]) => {
        params.set(`algorithm_params.${key}`, value.toString());
      });
    }
