    ...
    <div className={`left ${!isOpen ? 'collapsed' : ''}`}>
      ...
      {!isOpen && (
        <div
          ...
          onClick={onToggle}
          onMouseEnter={(e) => {
            (e.target as HTMLElement).style.backgroundColor = 'rgba(14, 165, 233, 0.1)';
            (e.target as HTMLElement).style.borderColor = 'var(--brand)';
          }}
          onMouseLeave={(e) => {
            (e.target as HTMLElement).style.backgroundColor = 'var(--panel)';
            (e.target as HTMLElement).style.borderColor = 'var(--border)';
          }}
          title="Expand Data Catalog"
        >
          ▶
        </div>
      )}
