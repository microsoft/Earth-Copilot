...
**Deploy your MCP Server using the [QUICK_DEPLOY.md](../../QUICK_DEPLOY.md) guide!**

After deployment, your MCP endpoints will be:
- **MCP Endpoint**: `https://YOUR-MCP-APP.azurecontainerapps.io`
- **API Documentation**: `https://YOUR-MCP-APP.azurecontainerapps.io/docs`
...
1. **Test with cURL/PowerShell** (No client needed!)
   ...
   Invoke-RestMethod -Uri "https://YOUR-MCP-APP.azurecontainerapps.io/tools/call" -Method Post -ContentType "application/json" -Body $body
   ```
...
2. **Connect Claude Desktop** - Add to `claude_desktop_config.json`:
   ```json
   {
     "mcpServers": {
       "earth-copilot": {
         "url": "https://YOUR-MCP-APP.azurecontainerapps.io"
       }
     }
   }
   ```
...
```
┌─────────────────┐         ┌─────────────────┐         ┌─────────────────┐
│   AI Client     │         │   MCP Server    │         │  Earth Copilot  │
...
## �🏗️ How to Consume the MCP Server
...
# 3. Get your endpoint
# Output: https://your-mcp-server.azurecontainerapps.io
```
...
.\deploy-apim.ps1 `
  ...
  -McpServerUrl "https://your-mcp-server.azurecontainerapps.io"

# Result:
# MCP Server: https://your-mcp-server.azurecontainerapps.io
...
```typescript
// Connect to MCP server (replace with your deployed URL)
const mcpClient = new MCPClient("https://YOUR-MCP-APP.azurecontainerapps.io");
...
# Direct HTTP API calls (replace with your deployed URL)
response = requests.post(
    "https://YOUR-BACKEND-APP.azurecontainerapps.io/api/query",
    json={
...
```python
# Register Earth-Copilot as MCP sub-agent (replace with your deployed URL)
earth_copilot_agent = {
    "name": "Earth-Copilot",
    "type": "mcp_server",
    "endpoint": "https://YOUR-MCP-APP.azurecontainerapps.io",
    "capabilities": [
...
### Environment Variables
```bash
# Backend API Configuration (set these to your deployed URLs)
EARTH_COPILOT_BASE_URL=https://YOUR-BACKEND-APP.azurecontainerapps.io
GEOINT_SERVICE_URL=https://YOUR-BACKEND-APP.azurecontainerapps.io

# Azure OpenAI (automatically configured by deployment)
AZURE_OPENAI_ENDPOINT=https://your-openai.openai.azure.com
AZURE_OPENAI_API_KEY=your-api-key
AZURE_OPENAI_DEPLOYMENT_NAME=gpt-4o

# Azure Maps (automatically configured by deployment)
