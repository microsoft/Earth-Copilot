...
try {
    ...
    if (-not $WhatIf) {
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Host "⚠️ Application Insights creation failed, continuing..." -ForegroundColor Yellow
        } else {
        ...
        if (-not $WhatIf) {
            ...
            if (Test-Path $apimDeployScript) {
                ...
                if ($LASTEXITCODE -eq 0) {
                    ...
                    Write-Host "⚠️ APIM deployment failed, continuing..." -ForegroundColor Yellow
                }
            } else {
                Write-Host "⚠️ APIM deployment script not found, skipping..." -ForegroundColor Yellow
            }
        }
    }
