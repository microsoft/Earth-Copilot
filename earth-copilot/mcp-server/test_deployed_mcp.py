...
class DeployedMCPTester:
    ...
    def test_comparison_analysis(self):
        ...
        try:
            ...
            data = response.json()
            print("✅ PASSED - Comparison analysis executed")
            print(f"   Result preview: {str(data)[:300]}...")
            
            self.test_results.append(("Comparison Analysis", True, None))
            return data
        except Exception as e:
            print(f"⚠️  SKIPPED - {str(e)} (May not be fully implemented)")
    ...
    def test_response_times(self):
        ...
        try:
            ...
            print("✅ PASSED - Response times measured")
            print(f"   Health check: {health_time:.2f}ms")
            print(f"   Tools list: {tools_time:.2f}ms")
            
            if health_time > 1000:
                print(f"   ⚠️  Warning: Health check is slow (>{health_time:.0f}ms)")
...
def main():
    if len(sys.argv) < 2:
        ...
        print("\nExamples:")
        print("  python test_deployed_mcp.py https://your-mcp-server.azurecontainerapps.io")
    ...
    if not tester.test_health():
        print("\n❌ Server health check failed. Cannot proceed with other tests.")
        print("\n💡 Troubleshooting:")
