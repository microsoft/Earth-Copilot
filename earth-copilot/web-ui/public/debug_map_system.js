...
window.MapDebugger = {
    ...
    log(level, category, message, data = null) {
        ...
        if (messageLevelIndex <= currentLevelIndex) {
            ...
            const prefix = `🗺️ [${timestamp}] [${level}] [${category}]`;
    ...
    async testTileUrl(url, tileInfo = {}) {
        ...
        try {
            ...
            if (success) {
                this.log('INFO', 'TILE_SUCCESS', `✅ Tile loaded successfully [${requestId}]`, {
                ...
                this.log('ERROR', 'TILE_FAILED', `❌ Tile failed [${requestId}]`, {
                    duration: `${duration}ms`,
                    status: response.status,
                    statusText: response.statusText,
                    error: errorText,
            ...
            this.log('ERROR', 'TILE_ERROR', `❌ Tile request error [${requestId}]`, {
                duration: `${duration}ms`,
                error: error.message,
                url: url.substring(0, 100) + '...'
            });
    ...
    generateReport() {
        ...
        console.group('🗺️ MAP DEBUGGING REPORT');
...
window.STACDebugger = {
    ...
    async testTilesFromTilejson(tilejsonData, feature, featureIndex) {
        ...
        for (const zoom of testZooms) {
            ...
            if (result.success) {
                window.MapDebugger.log('INFO', 'WORKING_TILE_FOUND', `✅ Found working tile for feature ${featureIndex}`, {
                    zoom,
                    x,
                    y,
...
console.log('🗺️ MAP DEBUGGING SYSTEM LOADED');
