...
export class TileUrlGenerator {
  ...
  static generateTileJsonUrl(options: TileUrlOptions): string {
    ...
    const format = options.format || getTileFormat(options.collection);
    const scale = options.scale || 1; // Use scale 1 to avoid @2x in tile URLs that cause 404s
    ...
    if (options.collection === 'sentinel-2-l2a' && 
      ...
      assets = ['visual'];
      console.log('🗺️ TileUrlGenerator: Corrected Sentinel-2 L2A assets from [red,green,blue] to [visual]');
    }
    ...
    params.set('collection', options.collection);
    // Remove tile_scale to avoid @2x suffix in tile URLs that cause 404s
