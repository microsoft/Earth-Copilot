...
export function prepareMultiTileData(data: SatelliteData): PreparedTileData[] {
  if (!data.all_tile_urls || data.all_tile_urls.length === 0) {
    console.warn('🗺️ [SatelliteDataHelpers] No multi-tile data available');
    return [];
  }

  const collection = getCollection(data);
  
  console.log(`🗺️ [SatelliteDataHelpers] Preparing ${data.all_tile_urls.length} tiles for multi-tile rendering`);
...
export function prepareSingleTileData(data: SatelliteData): PreparedTileData | null {
  ...
  if (!bounds) {
    console.warn('🗺️ [SatelliteDataHelpers] No bounds available for single-tile data');
    return null;
  }
  ...
  if (data.tilejson_url) {
    console.log('🗺️ [SatelliteDataHelpers] Using tilejson_url for single-tile rendering');
    return {
  ...
  if (data.tile_url) {
    console.log('🗺️ [SatelliteDataHelpers] Using direct tile_url for single-tile rendering');
    return {
  ...
  console.warn('🗺️ [SatelliteDataHelpers] No tile URL available for single-tile data');
  return null;
}
...
export function applyAssetFixes(tileUrl: string, collection: string): string {
  ...
  if (collection.toLowerCase().includes('sentinel-2-l2a')) {
    if (tileUrl.includes('assets=red') || 
      ...
      if (fixedUrl !== tileUrl) {
        console.log('🗺️ [SatelliteDataHelpers] Fixed Sentinel-2 L2A assets: [red,green,blue] → [visual]');
        return fixedUrl;
      }
    }
  }
