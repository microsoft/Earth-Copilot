...
export function startPerformanceTracking(operationId: string): void {
  ...
  log(
    ...
    `⏱️ Started tracking: ${operationId}`
  );
}
...
export function endPerformanceTracking(operationId: string): number {
  ...
  if (!startTime) {
    log(
      LogLevel.WARN,
      LogCategory.PERFORMANCE,
      `⚠️ No start time found for: ${operationId}`
    );
    return 0;
  }
  ...
  log(
    LogLevel.INFO,
    LogCategory.PERFORMANCE,
    `⏱️ ${operationId} completed in ${duration.toFixed(2)}ms`
  );
