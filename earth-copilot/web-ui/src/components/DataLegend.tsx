...
const DataLegend: React.FC<DataLegendProps> = ({ 
    ...
    const fetchColormap = async () => {
      try {
        ...
        if (data.success && data.css_gradient) {
          ...
          console.warn(`⚠️ Failed to load colormap for '${collection}', using fallback`);
          setColormapData(null);
        }
      } catch (error) {
  ...
  if (loading) {
    return (
      <div className="absolute bottom-8 left-4 z-[1000] bg-white rounded-lg shadow-lg p-4 min-w-[200px]">
