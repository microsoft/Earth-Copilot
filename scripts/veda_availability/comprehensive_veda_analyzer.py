            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"{self.base_url}/collections") as response:
                    if response.status == 200:
                        ...
                        logger.info(f"✅ Found {len(collections)} VEDA collections")
                        return collections
                    else:
                        logger.error(f"❌ Failed to fetch collections: HTTP {response.status}")
                        return []
    ...
    async def analyze_collection(self, collection_id: str, session: aiohttp.ClientSession) -> Dict[str, Any]:
        ...
        for strategy_name, strategy_config in self.test_strategies.items():
            try:
                ...
                logger.info(f"  📊 Testing strategy '{strategy_name}' for {collection_id}")
                ...
                async with session.post(self.search_url, json=query) as response:
                    if response.status == 200:
                        ...
                        logger.info(f"    ✅ Strategy '{strategy_name}': {len(features)} features found")
                        ...
                        logger.warning(f"    ❌ Strategy '{strategy_name}': HTTP {response.status}")
                ...
                collection_result["strategies"][strategy_name] = strategy_result
                collection_result["failed_strategies"].append(strategy_name)
                logger.error(f"    💥 Strategy '{strategy_name}': {e}")
        ...
        logger.info(f"📈 Collection {collection_id}: {collection_result['status']} ({len(collection_result['working_strategies'])}/{len(self.test_strategies)} strategies working)")
        return collection_result

    async def analyze_all_collections(self):
        """Analyze all VEDA collections"""
        logger.info("🚀 Starting comprehensive VEDA collection analysis...")
        
        # Fetch all collections first
        collections = await self.fetch_all_collections()
        if not collections:
            logger.error("❌ No collections found, aborting analysis")
            return
        ...
        logger.info(f"📚 Collected metadata for {len(self.collections_metadata)} collections")
            ...
            for collection in collections:
                ...
                if collection_id:
                    try:
                        ...
                        logger.error(f"💥 Failed to analyze {collection_id}: {e}")
...
"""
    ...
    def save_results(self, output_dir: str = "veda_analysis_results"):
        ...
        logger.info(f"📊 Found {len(working_profiles)} working VEDA collections")
