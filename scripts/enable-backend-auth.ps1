...
if (-not (Test-Path $envDir)) {
    Write-Host "⚠️  Environment directory not found: $envDir" -ForegroundColor Yellow
...
Write-Host "✏️  Adding authentication configuration..." -ForegroundColor Cyan
$envContent += ""
...
Write-Host "✅ Authentication configuration saved to: $envFile" -ForegroundColor Green
Write-Host ""

# Also update azd environment
Write-Host "🔧 Setting azd environment variables..." -ForegroundColor Cyan
try {
    ...
    Write-Host "✅ azd environment variables set" -ForegroundColor Green
} catch {
    Write-Host "⚠️  Could not set azd environment variables (azd may not be configured)" -ForegroundColor Yellow
}
...
if ($DeployAfterConfig) {
    Write-Host "🚀 Deploying backend with authentication enabled..." -ForegroundColor Yellow
    Write-Host ""
...
Write-Host ""
Write-Host "🔒 After deployment, your backend API will require Entra ID authentication!" -ForegroundColor Green
Write-Host ""
