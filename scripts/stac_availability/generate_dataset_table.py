...
def fetch_description_from_pc_api(collection_id: str) -> str:
    ...
    try:
        ...
        print(f"  ⚠️ Could not fetch description from PC API for {collection_id}: {e}")
        return None
...
def extract_description(description_file: Path, collection_id: str = None) -> str:
    ...
    if collection_id:
        print(f"  📡 Fetching description from Planetary Computer API for {collection_id}...")
...
def generate_markdown_table(datasets: List[Dict[str, Any]]) -> str:
    ...
    for dataset in datasets:
        ...
        md += f"| {'✅' if dataset['datetime_required'] else '❌'} "
        md += f"| {'✅' if dataset['location_required'] else '❌'} "
