...
Write-Host "🔒 Restricting access to specific users for app: $ClientId" -ForegroundColor Cyan
Write-Host ""

# Step 1: Get or create service principal
Write-Host "📝 Step 1: Getting service principal..." -ForegroundColor Yellow
try {
    ...
    Write-Host "❌ Error: Could not get/create service principal: $_" -ForegroundColor Red
    exit 1
}
Write-Host ""

# Step 2: Enable user assignment requirement
Write-Host "🔐 Step 2: Enabling user assignment requirement..." -ForegroundColor Yellow
try {
    az ad sp update --id $spObjectId --set appRoleAssignmentRequired=true
    Write-Host "✅ User assignment requirement enabled" -ForegroundColor Green
    Write-Host "   Now only assigned users can access the app." -ForegroundColor White
} catch {
    Write-Host "❌ Error enabling user assignment requirement: $_" -ForegroundColor Red
    exit 1
}
Write-Host ""
...
if ($UserEmails.Count -gt 0) {
    Write-Host "👥 Step 3: Assigning users..." -ForegroundColor Yellow
    ...
    foreach ($email in $UserEmails) {
        try {
            ...
            Write-Host "  ❌ Error adding user $email : $_" -ForegroundColor Red
        }
    }
    Write-Host ""
} else {
...
Write-Host "✅ ACCESS RESTRICTION COMPLETE!" -ForegroundColor Green
Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "🔓 To allow all tenant users again:" -ForegroundColor Yellow
