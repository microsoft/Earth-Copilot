...
logger = logging.getLogger("planetary-explorer-mcp")

class PlanetaryExplorerMCPServer:
    """MCP Server for Planetary-Explorer geospatial intelligence capabilities."""
    
    def __init__(self):
        """Initialize the MCP server with Planetary-Explorer capabilities."""
        self.server = Server("planetary-explorer")
        ...
        # Configuration
        self.base_url = os.getenv('PLANETARY_EXPLORER_BASE_URL', '')
        self.backend_api_url = os.getenv('PLANETARY_EXPLORER_API_URL', '')
        self.geoint_url = os.getenv('GEOINT_SERVICE_URL', '')
        
        # Setup MCP handlers
        self._setup_handlers()
        
        logger.info("Planetary-Explorer MCP Server initialized")
    ...
    def _setup_handlers(self):
        ...
        async def handle_list_tools() -> ListToolsResult:
            ...
            return ListToolsResult(
                tools=[
                    ...
                    Tool(
                        name="query_fabric_lakehouse",
                        description="Execute a SQL query against a Microsoft Fabric Lakehouse on behalf of the signed-in user. Use when the question requires structured data (tabular rows, metrics, aggregations) from the customer's Fabric workspace. The caller MUST have already selected a workspace + lakehouse (see /api/fabric/workspaces).",
                        inputSchema={
                            "type": "object",
                            "properties": {
                                "workspace_id": {"type": "string", "description": "Fabric workspace id (GUID)"},
                                "lakehouse_id": {"type": "string", "description": "Lakehouse / SQL endpoint id"},
                                "sql": {"type": "string", "description": "Read-only SELECT statement."}
                            },
                            "required": ["workspace_id", "lakehouse_id", "sql"]
                        }
                    ),
                    Tool(
                        name="search_fabric_documents",
                        description="Semantic search across the customer's Fabric-indexed document corpus (e.g. Azure AI Search index populated from a OneLake folder). Use BEFORE imagery analysis when the question is about methodology, prior reports, or unstructured references.",
                        inputSchema={
                            "type": "object",
                            "properties": {
                                "workspace_id": {"type": "string", "description": "Optional Fabric workspace id"},
                    ...
                    Tool(
                        name="audit_site",
                        description=(
                            "Produce a regulator-grade siting dossier for a proposed AI data center "
                            "at a specific (lat, lng) and claimed megawatt load. Integrates ALL THREE "
                            "GeoAI Accelerator sources in a single call: Fabric Lakehouse (power, "
                            "water, parcel, competing data centers), Microsoft Planetary Computer "
                            "(land cover + elevation rasters), and Azure AI Search (permitting / "
                            "interconnection precedent corpus). Returns scores per dimension plus "
                            "structured evidence rows with `kind` discriminator. Use this whenever "
                            "the user asks to audit / evaluate / score a specific site or pin."
                        ),
                        inputSchema={
                            "type": "object",
                            "properties": {
                                ...
                                "claimed_mw": {
                                    "type": "number",
                                    "description": "Claimed peak load in MW",
                                    "default": 200,
                                },
                            },
                            "required": ["lat", "lng"]
        ...
        async def handle_call_tool(name: str, arguments: Dict[str, Any]) -> CallToolResult:
            ...
            try:
                ...
                elif name == "query_fabric_lakehouse":
                    return await self._query_fabric_lakehouse(arguments)
                elif name == "search_fabric_documents":
                    return await self._search_fabric_documents(arguments)
                elif name == "audit_site":
                    return await self._audit_site(arguments)
                else:
    ...
    async def _query_fabric_lakehouse(self, arguments: Dict[str, Any]) -> CallToolResult:
        """Forward to the backend /api/fabric/query endpoint."""
        import aiohttp
        ws = (arguments.get("workspace_id") or "").strip()
        lh = (arguments.get("lakehouse_id") or "").strip()
        sql = (arguments.get("sql") or "").strip()
        if not (ws and lh and sql):
            return CallToolResult(content=[TextContent(type="text", text="Error: workspace_id, lakehouse_id, sql required")])
        url = f"{self.backend_api_url.rstrip('/')}/api/fabric/query"
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json={"workspace_id": ws, "lakehouse_id": lh, "sql": sql}, timeout=aiohttp.ClientTimeout(total=60)) as resp:
                    body = await resp.text()
                    if resp.status != 200:
                        return CallToolResult(content=[TextContent(type="text", text=f"Fabric query failed ({resp.status}): {body[:300]}")])
                    return CallToolResult(content=[TextContent(type="text", text=body[:8000])])
        except Exception as e:
            logger.error(f"Fabric MCP tool error: {e}")
            return CallToolResult(content=[TextContent(type="text", text=f"Fabric unreachable: {e}")])

    async def _search_fabric_documents(self, arguments: Dict[str, Any]) -> CallToolResult:
        """Forward to the backend /api/fabric/search_documents endpoint."""
        import aiohttp
        ...
        url = f"{self.backend_api_url.rstrip('/')}/api/fabric/search_documents"
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(url, json=payload, timeout=aiohttp.ClientTimeout(total=30)) as resp:
                    if resp.status != 200:
                        body = await resp.text()
                        return CallToolResult(content=[TextContent(type="text", text=f"Fabric doc search failed ({resp.status}): {body[:300]}")])
        ...
        if not hits:
            return CallToolResult(content=[TextContent(type="text", text="No documents found.")])
        lines: List[str] = []
        for i, h in enumerate(hits[:5], 1):
            title = h.get("title") or h.get("metadata_title") or h.get("id") or "doc"
            snippet = (h.get("content") or h.get("excerpt") or "").strip()
            if len(snippet) > 200:
                snippet = snippet[:197] + "..."
            lines.append(f"{i}. {title}" + (f" — {snippet}" if snippet else ""))
        return CallToolResult(content=[TextContent(type="text", text="\n".join(lines))])

    async def _audit_site(self, arguments: Dict[str, Any]) -> CallToolResult:
        """Forward to the backend /api/sites/audit endpoint and render a compact summary."""
        import aiohttp
        try:
            lat = float(arguments["lat"])
            lng = float(arguments["lng"])
        except (KeyError, TypeError, ValueError):
            return CallToolResult(content=[TextContent(type="text", text="Error: 'lat' and 'lng' (numbers) required")])
        claimed_mw = float(arguments.get("claimed_mw") or 200)
        url = f"{self.backend_api_url.rstrip('/')}/api/sites/audit"
        payload = {"lat": lat, "lng": lng, "claimed_mw": claimed_mw}
        try:
            async with aiohttp.ClientSession() as session:
                # Audit fans out to OneLake + MPC + AI Search; keep timeout generous.
                async with session.post(url, json=payload, timeout=aiohttp.ClientTimeout(total=120)) as resp:
                    body = await resp.text()
                    if resp.status != 200:
                        return CallToolResult(content=[TextContent(type="text", text=f"Site audit failed ({resp.status}): {body[:300]}")])
                    # Cap returned text — full JSON dossier is also the natural payload.
                    return CallToolResult(content=[TextContent(type="text", text=body[:12000])])
        except Exception as e:
            logger.error(f"Site audit MCP tool error: {e}")
            return CallToolResult(content=[TextContent(type="text", text=f"Backend unreachable: {e}")])

    async def _analyze_satellite_imagery(self, arguments: Dict[str, Any]) -> CallToolResult:
        """Analyze satellite imagery using Planetary-Explorer services."""
        ...
        if self.semantic_translator:
            ...
            translation_result = {
                "service": "planetary-explorer",
                "parameters": arguments
            }
        
        # Simulate analysis result (in production, call actual Planetary-Explorer API)
        result = {
...
You use satellite data and geospatial analysis to understand environmental processes, monitor ecosystem changes, and support evidence-based environmental policy and conservation efforts."""
    ...
    async def run(self, transport):
        """Run the MCP server."""
        logger.info("Starting Planetary-Explorer MCP Server...")
        async with mcp.server.stdio.stdio_server() as (read_stream, write_stream):
            await self.server.run(
                read_stream,
                write_stream, 
                InitializationOptions(
                    server_name="planetary-explorer",
...
async def main():
    ...
    server = PlanetaryExplorerMCPServer()
