"""
MCP-to-HTTP Bridge for Planetary-Explorer

This module provides an HTTP API that bridges to the MCP (Model Context Protocol) server,
allowing standard HTTP clients to interact with Planetary-Explorer's MCP capabilities.
...
logger = logging.getLogger("planetary-explorer-mcp-bridge")

app = FastAPI(
    title="Planetary-Explorer MCP Bridge",
...
PLANETARY_EXPLORER_BASE_URL = os.getenv("PLANETARY_EXPLORER_BASE_URL", "")
    ...
    async def call_tool(self, name: str, arguments: Dict[str, Any]):
        ...
        result = {
            ...
            "result": {
                ...
                "confidence": 0.85,
                "visualization_url": f"{PLANETARY_EXPLORER_BASE_URL}/?query={arguments.get('query', '')}"
            }
        }
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting Planetary-Explorer MCP Bridge on {host}:{port}")
