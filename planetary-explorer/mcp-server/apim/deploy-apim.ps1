# Azure API Management Deployment Script for Planetary-Explorer MCP Server
# 
...
Write-Host " Deploying Planetary-Explorer MCP API Management" -ForegroundColor Cyan
...
try {
    ...
    $deploymentName = "planetary-explorer-mcp-apim-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
    ...
    az apim product create `
        ...
        --product-id "planetary-explorer-starter" `
        --product-name "Planetary-Explorer Starter" `
        --description "Starter plan for Planetary-Explorer MCP API" `
    ...
    az apim product create `
        --resource-group $ResourceGroupName `
        --service-name $ApimServiceName `
        --product-id "planetary-explorer-professional" `
        --product-name "Planetary-Explorer Professional" `
        --description "Professional plan for Planetary-Explorer MCP API" `
        --subscription-required true `
    ...
    az apim product api add `
        ...
        --service-name $ApimServiceName `
        --product-id "planetary-explorer-starter" `
        --api-id "planetary-explorer-mcp-api" `
        --output table
    ...
    az apim group create `
        --resource-group $ResourceGroupName `
        --service-name $ApimServiceName `
        --group-id "planetary-explorer-developers" `
        --display-name "Planetary-Explorer Developers" `
        --description "Developers using Planetary-Explorer MCP API" `
        --output table
    ...
    if ($instrumentationKey) {
        ...
        az apim logger create `
            --resource-group $ResourceGroupName `
            --service-name $ApimServiceName `
            --logger-id "planetary-explorer-logger" `
            --logger-type "applicationInsights" `
            --description "Planetary-Explorer MCP API Logger" `
            --credentials "instrumentationKey=$instrumentationKey" `
            --output table
    }
    
    Write-Host ""
    Write-Host " Planetary-Explorer MCP APIM Setup Complete!" -ForegroundColor Green
    ...
    $configFile = "planetary-explorer-mcp-apim-config.json"
...
Write-Host ""
Write-Host " Planetary-Explorer MCP APIM is ready for use!" -ForegroundColor Green