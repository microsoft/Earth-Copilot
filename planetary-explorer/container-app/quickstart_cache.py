...
QUICKSTART_QUERIES: Dict[str, Dict[str, Any]] = {
    ...
    "show coastal land cover changes in california": {
        ...
        "location": "California",
        "bbox": [-124.48, 32.53, -114.13, 42.01],
        "description": "30m NOAA Coastal Change Analysis Program land cover",
        "dataset": "NOAA C-CAP",
        "intent": "stac"
    },
    ...
    # NOTE: Bare location names (e.g. "jalalabad, afghanistan") are NOT cached
    # here. They must flow through RouterAgent so its deterministic location
    # pre-check returns navigate_to (pan the map, no STAC load) for any input
    # that is just a place. Cache entries override the agent and would force
    # those queries into a wrong STAC tile load. Users who want imagery should
    # ask "show HLS imagery of Jalalabad" — that has explicit data intent and
    # the RouterAgent + semantic translator will route it to STAC correctly.
