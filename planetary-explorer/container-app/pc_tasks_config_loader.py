...
class UnifiedPCLoader:
    ...
    def __init__(self, json_path: Optional[str] = None):
        ...
        # When a collection entry in pc_rendering_config.json carries a
        # ``presets`` block (STAC-render-extension-shaped dict of named
        # preset specs) we cache the raw dict here so that intent-aware
        # lookups can score presets at call time. The flat ``rendering``
        # block continues to populate ``_rendering_cache`` as the default.
        self._presets_cache: Dict[str, Dict[str, Any]] = {}
        # Optional ``msft:render_default`` carried alongside ``presets``.
        self._preset_defaults_cache: Dict[str, str] = {}
        ...
        try:
            ...
            for collection_id, config_data in collections_data.items():
                try:
                    ...
                    # Cache named-preset block when present. Same shape as the
                    # STAC render extension: {<name>: {assets,...}, ...}.
                    presets_block = config_data.get('presets')
                    if isinstance(presets_block, dict) and presets_block:
                        self._presets_cache[collection_id] = presets_block
                        default_name = config_data.get('msft:render_default')
                        if isinstance(default_name, str) and default_name:
                            self._preset_defaults_cache[collection_id] = default_name
    ...
    def get_rendering_config(
        ...
        When ``query_context`` is provided AND the collection has a cached
        ``presets`` block AND a preset's key/title/description matches the
        query (score > 0), return a RenderingConfig built from that preset.
        Falls back to the flat default config in every other case.

        This is the Public-side counterpart to the live STAC ``renders``
        intent matcher in :mod:`hybrid_rendering_system` -- a collection
        with no live ``renders`` block on Public PC (the common case for
        sentinel-2-l2a today) can still serve intent-matched presets when
        we publish them locally here.
        """
        if not self._initialized:
            logger.warning(f"  PC loader not initialized, cannot load {collection_id}")
            return None

        # Resolve aliases once (e.g. sentinel-2-l2a -> sentinel-2).
        resolved_id = collection_id if collection_id in self._rendering_cache else self._resolve_collection_id(collection_id)
        if resolved_id != collection_id:
            logger.info(f" Collection ID alias: {collection_id} -> {resolved_id}")

        # Intent-aware preset selection. Only fires when caller passed a
        # query AND the entry has presets AND a preset actually matches by
        # intent; otherwise we fall through to the default config so we
        # never break callers that don't care about presets.
        if query_context and resolved_id in self._presets_cache:
            preset_cfg = self._pick_preset_config(resolved_id, query_context)
            if preset_cfg is not None:
                return preset_cfg

        return self._rendering_cache.get(resolved_id)

    def _pick_preset_config(
        self,
        resolved_id: str,
        query_context: str,
    ) -> Optional[RenderingConfig]:
        """Return a RenderingConfig built from the intent-matched preset.

        Uses :func:`hybrid_rendering_system._pick_preset_by_intent` so the
        synonym table and scoring are identical to the live STAC renders
        path. Returns None when no preset scores > 0 (caller falls back to
        the default flat config).
        """
        try:
            # Local import to avoid a circular dependency at module load.
            from hybrid_rendering_system import _pick_preset_by_intent, _preset_to_rendering_config  # type: ignore
        except Exception as exc:  # pragma: no cover - defensive
            logger.debug(f"[PRESETS] picker import failed for {resolved_id}: {exc}")
            return None
        presets = self._presets_cache.get(resolved_id)
        if not presets:
            return None
        name, preset, intent_matched = _pick_preset_by_intent(presets, query_context)
        if not preset or not intent_matched:
            return None
        cfg = _preset_to_rendering_config(resolved_id, name or "default", preset)
        if cfg:
            cfg.notes = (cfg.notes or "") + f" [pc_rendering_config presets, intent-matched={name}]"
        return cfg
...
def get_pc_rendering_config(
    ...
    Args:
        ...
        query_context: Optional user query. When provided, collections
            with a ``presets`` block in pc_rendering_config.json get
            intent-aware preset selection (otherwise the flat default
            config is returned).
    ...
    """
    loader = _get_loader()
    return loader.get_rendering_config(collection_id, query_context=query_context)
