 ...
 *
 * We send id_token (not access_token) for backend authentication because:
 *   1. id_token's `aud` claim equals our app's client_id — the backend can
 *      configure a fixed allow-list and reject anything else.
 *   2. id_tokens are EXPLICITLY designed to be signature-verified by client
 *      apps (RFC 7519 § 4 + OIDC core spec). Access tokens — particularly
 *      Microsoft Graph tokens — embed an opaque nonce in their signature
 *      that makes them unverifiable by anyone except their target API.
 *   3. The backend no longer does user-OBO for Fabric (it uses Managed
 *      Identity), so we don't need a delegated access_token at all.
 *
...
export async function getAuthToken(): Promise<string | null> {
  ...
  try {
    ...
    // Prefer id_token — it's the OpenID identity assertion, locally
    // verifiable by the backend. access_token is only useful if the
    // backend needs to call another API on behalf of the user, which we
    // don't do anymore (Fabric access uses the backend's own MI).
